...
from offline_rl_config import args_cheetah_vel, args_cheetah_dir, args_ant_dir, args_hopper_param, args_walker_param, args_sparse_data, args_point_robot_v1, args_humanoid_dir
import numpy as np

from models.encoder import RNNEncoder, MLPEncoder, ContextClassifier, TaskClassifier
...
class FOCAL:
	...
	def __init__(self, args, train_dataset, train_goals, eval_dataset, eval_goals):
		...
		# self.agent.load_state_dict(torch.load("/root/unicorn/logs/AntDir-v0/classifier_seed1/models/agent1000.pt", map_location=ptu.device))
	...
	def update(self, tasks, _iter):
		...
		for update in range(self.args.rl_updates_per_iter): 
			...
			classifier_encoding, classifier_loss = self.update_classifier(tasks, context)
			...
			rl_losses["classifier_loss"] = classifier_loss.item()
			if self.args.log_train_time:
				_t_now = time.time()
	...
	# 将数值映射到文本标记的函数
	def float_to_token(self, value, precision=2):
		return f"{value:.{precision}f}"

	def discretize(self, value, min_val, max_val, num_bins=256):
		bin_index = ((value - min_val) / (max_val - min_val) * (num_bins - 1)).__round__()
		return str(bin_index)

	# 将每个transition拼接成一个文本字符串
	def preprocess_transition(self, transition):
		s = transition[:self.args.obs_dim] 
		a = transition[self.args.obs_dim: self.args.obs_dim + self.args.action_dim]
		s_prime = transition[self.args.obs_dim + self.args.action_dim: self.args.obs_dim * 2 + self.args.action_dim]
		r = transition[-2]
		done = transition[-1]
		s_tokens = ' '.join([self.discretize(v.item(), min_val=-20, max_val=20) for v in s])
		a_tokens = ' '.join([self.discretize(v.item(), min_val=-1, max_val=1) for v in a])
		r_token = self.discretize(r.item(), min_val=-10, max_val=10)
		d_token = self.float_to_token(done.item())
		s_prime_tokens = ' '.join([self.float_to_token(v.item()) for v in s_prime])
		return f"s: [{s_tokens}] a: [{a_tokens}] r: [{r_token}]"

	def turn_to_embedding(self, transitions, size):
		preprocessed_transitions = [self.preprocess_transition(t) for t in transitions]
		# compose to trajectory	
		preprocessed_trajectory = []
		for i in range(size):
			traj_desc = "trajectory is:"
			for j in range(self.args.last_n):
				traj_desc += preprocessed_transitions[i * self.args.last_n + j] + " "
			preprocessed_trajectory.append(traj_desc)
		inputs = self.tokenizer(preprocessed_trajectory, padding=True, return_tensors="pt").to(ptu.device)
		with torch.no_grad():
	...
	def update_classifier(self, tasks, context):
		...
		obs_context, actions_context, rewards_context, next_obs_context, terms_context = obs_context[-self.args.last_n:].permute(1, 0, 2), actions_context[-self.args.last_n:].permute(1, 0, 2), rewards_context[-self.args.last_n:].permute(1, 0, 2), next_obs_context[-self.args.last_n:].permute(1, 0, 2), terms_context[-self.args.last_n:].permute(1, 0, 2)

		transitions = torch.cat((obs_context, actions_context, next_obs_context, rewards_context, terms_context), dim=-1).reshape(self.args.meta_batch * self.args.last_n, -1)
		task_embeddings = self.turn_to_embedding(transitions, self.args.meta_batch)
  
		encoding, prediction = self.context_classifier(task_embeddings)
		...
		return encoding, cross_entropy_loss
	...
	def evaluate(self, trainset=False, ood=False):
		...
		for task in eval_env.unwrapped.get_all_task_idx():
			...
			'''
			obs_context, actions_context, rewards_context, next_obs_context, terms_context = obs_context[-self.args.last_n:].permute(1, 0, 2), actions_context[-self.args.last_n:].permute(1, 0, 2), rewards_context[-self.args.last_n:].permute(1, 0, 2), next_obs_context[-self.args.last_n:].permute(1, 0, 2), terms_context[-self.args.last_n:].permute(1, 0, 2)
			transitions = torch.cat((obs_context, actions_context, next_obs_context, rewards_context, terms_context), dim=-1).reshape(1 * self.args.last_n, -1)
			task_embeddings = self.turn_to_embedding(transitions, 1)
			task_desc, _ = self.context_classifier(task_embeddings)
